...
export const MAIN_THEME_STORAGE_KEY = 'shadcn-studio.main-theme.v2'
export const LEGACY_MAIN_THEME_STORAGE_KEY = 'shadcn-studio.main-theme.v1'
...
const BUILT_IN_THEME_PAIR_DEFINITION_MAP = new Map(
  BUILT_IN_THEME_PAIR_DEFINITIONS.map((definition) => [
    definition.name,
    definition,
  ]),
)
...
> = {
  light: new Map(
    DEFAULT_THEME_PALETTE_BY_MODE.light.colorPairs.map((pair) => [
      pair.name,
      pair,
    ]),
  ),
  dark: new Map(
    DEFAULT_THEME_PALETTE_BY_MODE.dark.colorPairs.map((pair) => [
      pair.name,
      pair,
    ]),
  ),
...
function getThemeVariablesWithOverrides(
  ...
  const customVariables = getCustomThemeColorPairs(theme, mode).flatMap(
    (pair) => {
      const tokenNames = getThemePairTokenNames(pair)
      return [
        [
          tokenNames.colorToken,
          variables.get(tokenNames.colorToken) ?? pair.color,
        ],
        [
...
function synchronizeThemePalettes(
  ...
  const normalizedLight = normalizeThemePalette(
    lightPalette.colorPairs,
    'light',
  )
  ...
  const darkBuiltInPairs = normalizedDark.colorPairs.filter(
    (pair) => !pair.isCustom,
  )

  const lightCustomPairs = normalizedLight.colorPairs.filter(
    (pair) => pair.isCustom,
  )
  const darkCustomPairs = normalizedDark.colorPairs.filter(
    (pair) => pair.isCustom,
  )
  ...
  const darkCustomPairByName = new Map(
    darkCustomPairs.map((pair) => [pair.name, pair]),
  )
...
function normalizeMainTheme(value: unknown): MainTheme | null {
  ...
  const legacyV1Pairs = parseThemeColorPairsFromUnknown(
    value.colorPairs,
    'light',
  )
  if (legacyV1Pairs) {
...
export function getBuiltInThemeColorPairs(
  ...
  return getThemePalette(theme, mode).colorPairs.filter(
    (pair) => !pair.isCustom,
  )
}
...
export function updateThemeColorPair(
  ...
  updates: Partial<
    Pick<
      ThemeColorPair,
      'label' | 'color' | 'foreground' | 'includeInButtonVariant'
    >
  >,
): MainTheme {
  const normalizedName = normalizeThemePairName(pairName)
  ...
  const updatePalettePairs = (
    ...
    pairs.map((pair) => {
      ...
      return {
        ...
        color: shouldUpdateColors
          ? ensureThemeColor(updates.color, pair.color)
          : pair.color,
